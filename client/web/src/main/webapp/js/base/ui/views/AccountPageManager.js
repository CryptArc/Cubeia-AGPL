...
Poker.AccountPageManager = Class.extend({
    ...
    currencyChanged : function(currencyCode) {
        this.selectedCurrency = currencyCode;
        this.updateAccounting(Poker.AppCtx.getProfileManager().myPlayerProfile);
    ...
    onBonusInfo : function(profile) {
        ...
        var displayAccounts = [];
        $(".profile-account-balance").removeClass("with-bonus");
        $(".profile-current-balance").html("");
        $(".profile-current-bonus").html("");
        $(".profile-current-balance-name").html("");
        ...
        $.each(profile.accounts,function(i,a){
            ...
            if(a.currencyCode == self.selectedCurrency && a.role=="MAIN") {
                var currency = Poker.OperatorConfig.getCurrencyMap().get(a.currencyCode);
                $(".profile-current-balance-name").html(currency.longName + " Balance");
                $(".profile-current-balance").html(a.formattedBalance);
            }
            if(a.currencyCode == self.selectedCurrency && a.role=="BONUS") {
                $(".profile-account-balance").addClass("with-bonus");
