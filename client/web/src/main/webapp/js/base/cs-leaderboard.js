(function ( $ ) {
    ...
    $.fn.cs_leaderboard = function(opts) {
        ...
        return this.each(function(el){
            ...
            var leaderboard = new Leaderboard($(this),id,nrOfItems,opts);
            leaderboard.start();
        });
    };
    ...
    var Leaderboard = function(element,id,nrOfItems,opts) {
        var self = this;
        var playerApi = Poker.AppCtx.getPlayerApi();
        ...
        var fetchLeaderboard = function() {
            ...
            playerApi.requestLeaderboard(self.id,self.opts.global,function(data){
                if(data.length == 0 ) {
        ...
        this.displayError = function() {
            self.element.html("Unable to fetch leaderboard");
        };
        this.shuffleItems = function(data) {
        ...
        this.updateLeaderboard = function(data) {
            ...
            $.each(data.entries, function(i,e){
                playerApi.requestPlayerProfile(e.playerId,null,
                    function(profile){
                        count--;
        ...
        this.displayItems = function(data) {
            ...
            for(var i = 0; i<nrOfItems && i<data.entries.length; i++) {
        ...
        this.animateRemove = function(playerId) {
           var toRemove = this.element.find(".player-"+playerId);
           cssUtils.addTransitionCallback(toRemove,function(){
               cssUtils.removeTransitionCallback(toRemove);
               toRemove.remove();
           });
           cssUtils.setTranslate3d(toRemove,0,this.element.height(),0,"px");
        };
        this.animate = function(player, direction) {
            var self = this;
            if(direction.from == -1) {
                var pe = $(this.template(player));
                cssUtils.setTranslate3d(pe,0,self.element.height(),0,"px");
                if(direction.first==false) {
                    pe.find(".arrow").addClass("arrow-up");
                }
                this.element.append(pe);
                setTimeout(function(){
                    cssUtils.setTranslate3d(self.element.find(".player-"+player.playerId),0,67*player.position,0,"px");
                },50);
                ...
                cssUtils.setTranslate3d(el,0,67*player.position,0,"px");
