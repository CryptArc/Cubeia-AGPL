...
var utf8=utf8||{};
FIREBASE=FIREBASE||{};
FIREBASE.ByteArray=function(array){var MAX_SIGNED_INT64=9007199254740992;
...
this.readLong=function(){this.checkBuffer(8);
var negate=this.buffer[this.position]&128,x=0,carry=1;
var i,v,m;
for(i=7,m=1;
i>=0;
i--,m*=256){v=this.buffer[i+this.position];
if(negate){v=(v^255)+carry;
carry=v>>8;
v=v&255
}x+=v*m
}if(x>MAX_INT_64){throw new RangeError("Precision lost when converting 64bit number")
}this.position+=8;
return negate?-x:x
};
...
this.twoCompliment=function(buffer){var carry=1;
var v,i;
for(i=7;
i>=0;
i--){v=(buffer[i]^255)+carry;
buffer[i]=v&255;
carry=v>>8
}};
this.writeLong=function(value){var negative;
var buffer=[0,0,0,0,0,0,0,0];
var lowPart;
var i;
if(value>MAX_SIGNED_INT64){throw new RangeError("value to big for long")
}else{if(value<MIN_SIGNED_INT64){throw new RangeError("value to small for long")
}}negative=value<0;
...
for(i=7;
i>=0;
i--){buffer[i]=(lowPart&255);
lowPart=i===4?value:lowPart>>>8
}if(negative){this.twoCompliment(buffer)
}for(i=0;
i<buffer.length;
i++){this.buffer.push(buffer[i])
}};
...
var previous=0;
var byteNum=0;
while(i<input.length){current=input[i]<0?input[i]+256:input[i];
byteNum=i%3;
...
var _reportDisconnected=function(){_statusCallback(FIREBASE.ConnectionStatus.DISCONNECTED,"cometd");
_firstConnect=true
};
...
var _unsubscribe=function(){_instance.unsubscribe("/service/client")
};
...
_instance.addListener("/meta/disconnect",function(message){if(message.successful){_connected=false
}_reportDisconnected()
});
this.close=function(){_instance.clearSubscriptions();
...
var _handleLoginResponse=function(loginResponse){if(_loginCallback){var sessionToken=null;
...
var _handleDisconnect=function(type){if(_cancelled){return
}if(!_reconnecting){if(FIREBASE.ReconnectStrategy.MAX_ATTEMPTS===0){_statusCallback(FIREBASE.ConnectionStatus.DISCONNECTED,0,"Disconnected",type);
return
}else{_statusCallback(FIREBASE.ConnectionStatus.DISCONNECTED,0,"Disconnected",type);
_reconnecting=true;
...
var _handlePacket=function(protocolObject){switch(protocolObject.classId){case FB_PROTOCOL.ForcedLogoutPacket.CLASSID:if(_packetCallback){_packetCallback(protocolObject)
}if(_cometd===true){_ioAdapter.close();
if(_reconnecting){clearTimeout(_reconnectTimer)
}_statusCallback(FIREBASE.ConnectionStatus.DISCONNECTED,0,"Disconnected","cometd");
_cancelled=true
}break;
case FB_PROTOCOL.PingPacket.CLASSID:console.log("received ping from firebase");
...
this.close=function(){try{this.cancel();
_ioAdapter.close()
}catch(e){console.log("exception thrown when closing connection")
}};
...
var IoAdapterClass;
_cancelled=false;
if(ioAdapterName==="FIREBASE.CometdAdapter"){_cometd=true
}secure=secure||false;
...
}_ioAdapter.connect(function(status,connectionType){if(status===FIREBASE.ConnectionStatus.DISCONNECTED){_handleDisconnect(connectionType)
...
_socket.onclose=function(){_statusCallback(FIREBASE.ConnectionStatus.DISCONNECTED,"socket")
}
};
this.connect=function(statusCallback,dataCallback){_statusCallback=statusCallback;
_dataCallback=dataCallback;
...
this.close=function(){_socket.close()
}
};