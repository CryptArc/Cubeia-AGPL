...
public class TurkishHandComparatorTest {


    private TurkishHandComparator createComparatorByLowestRank(Rank lowestRank, int playersInHand) {
        TurkishHandStrengthEvaluator evaluator = new TurkishHandStrengthEvaluator(lowestRank);
        return new TurkishHandComparator(evaluator, playersInHand);
    }

    @Test
    public void testShortHandsOneCard() {
        TurkishHandComparator comp = createComparatorByLowestRank(Rank.EIGHT, 3);
    ...
    public void testShortHandsTwoCards() {
        TurkishHandComparator comp = createComparatorByLowestRank(Rank.SEVEN, 4);
    ...
    public void testThreeOfAKind() {
        TurkishHandComparator comp = createComparatorByLowestRank(Rank.EIGHT, 4);
        // three of a kind beats straight
        assertTrue(comp.compare(new Hand("AS AC AD 9S 8D"), new Hand("8S 9C TH JS QH")) > 0);
        assertTrue(comp.compare(new Hand("9S 9C 9D JS 8D"), new Hand("AS 8C 9H TS JH")) > 0);

        // three of a kind beats two pair
        assertTrue(comp.compare(new Hand("AS AC AD 9S 8D"), new Hand("AS AC KH KS JH")) > 0);
        assertTrue(comp.compare(new Hand("9S 9C 9D JS 8D"), new Hand("AS AC KH KS JH")) > 0);

        // three of a kind beats pair
        assertTrue(comp.compare(new Hand("AS AC AD 9S 8D"), new Hand("AS AC KH QS JH")) > 0);
        assertTrue(comp.compare(new Hand("9S 9C 9D JS 8D"), new Hand("AS AC KH QS JH")) > 0);

        // three of a kind beats high card
        assertTrue(comp.compare(new Hand("9S 9C 9D JS 8D"), new Hand("AS KH QS JD 9H")) > 0);

        // higher rank three of a kind wins
        assertTrue(comp.compare(new Hand("AS AC AD JS 8D"), new Hand("QS QC QD 7S 8D")) > 0);
        assertTrue(comp.compare(new Hand("9S 9C 9D JS 8D"), new Hand("8S 8C 8D AS KD")) > 0);
