...
public class BettingRound implements Round, BettingRoundContext {
	...
	void raise(PokerPlayer player, long amount) {
		...
		System.out.println(" * amount["+amount+"] LastBetSize["+lastBetSize+"] lastValidBetLevel["+nextValidRaiseLevel+"] AllInBet["+allIn+"] belowMinBet["+belowMinBet+"]");
		...
		if (belowMinBet && allIn) {
			...
			System.out.println("Next valid raise level updated from "+oldValid+" to "+nextValidRaiseLevel);
		}
