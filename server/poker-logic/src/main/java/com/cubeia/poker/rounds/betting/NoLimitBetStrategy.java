...
public class NoLimitBetStrategy implements BetStrategy {
	...
	public long getMinRaiseToAmount(PokerPlayer player) {
		...
		System.out.println("Min Raise, HighestBet["+bettingRoundContext.getHighestBet()+"] LastBetSize["+bettingRoundContext.getSizeOfLastBetOrRaise()+"] NextValidRaiseLevel["+bettingRoundContext.getNextValidRaiseLevel()+"]");
		...
		System.out.println("Cost for max raise: "+cost+", affordable cost: "+affordableCost+", player betstack: "+player.getBetStack());
