...
//    private HashMultimap<Integer, Card> playerPublicCards;
	...
	public void visit(BettingRound bettingRound) {
		...
		if (isHandFinished()) {
		    exposeShowdownCards();
			HandResult handResult = new HandResultCreator(new TelesinaHandStrengthEvaluator(getDeckLowestRank())).createHandResult(state.getCommunityCards(), handResultCalculator, state.getPotHolder(), state.getCurrentHandPlayerMap());
