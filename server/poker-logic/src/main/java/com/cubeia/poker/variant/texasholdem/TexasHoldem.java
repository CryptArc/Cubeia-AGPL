...
public class TexasHoldem implements GameType, RoundVisitor {
	...
	public String toString() {
		return "TexasHoldem, current round[" + currentRound + "] roundId["
				+ roundId + "] ";
	}
	...
	private void initHand() {
		deck = new StandardDeck(new Shuffler<Card>(rngProvider.getRNG()),
				new IndexCardIdGenerator());
	...
	private void startBettingRound() {
		...
		currentRound = new BettingRound(this,
				blindsInfo.getDealerButtonSeatId(),
				new DefaultPlayerToActCalculator());
		roundId++;
	}
	...
	private void handleCanceledHand() {
		state.notifyHandFinished(new HandResult(),
				HandEndStatus.CANCELED_TOO_FEW_PLAYERS);
	}

	private void moveChipsToPot() {

		state.getPotHolder().moveChipsToPot(
				state.getCurrentHandSeatingMap().values());
	...
	public void requestAction(ActionRequest r) {
		if (blindRequested(r) && state.isTournamentTable()) {
			state.getServerAdapter().scheduleTimeout(
					state.getTimingProfile().getTime(
							Periods.AUTO_POST_BLIND_DELAY));
		} else {
	...
	public void scheduleRoundTimeout() {
		log.debug("scheduleRoundTimeout in: "
				+ state.getTimingProfile().getTime(Periods.RIVER));
		state.getServerAdapter().scheduleTimeout(
				state.getTimingProfile().getTime(Periods.RIVER));
	}

	private boolean blindRequested(ActionRequest r) {
		return r.isOptionEnabled(PokerActionType.SMALL_BLIND)
				|| r.isOptionEnabled(PokerActionType.BIG_BLIND);
	}
	...
	public String getStateDescription() {
		return currentRound == null ? "th-round=null" : currentRound.getClass()
				+ "_" + currentRound.getStateDescription();
	}
	...
	public void visit(BettingRound bettingRound) {
		...
		if (isHandFinished()) {
			...
			List<Integer> playerRevealOrder = new RevealOrderCalculator()
					.calculateRevealOrder(state.getCurrentHandSeatingMap(),
							state.getLastPlayerToBeCalled(),
							playerAtDealerButton);
			HandResult handResult = new HandResultCreator(
					new TexasHoldemHandCalculator()).createHandResult(
					state.getCommunityCards(), handResultCalculator,
					state.getPotHolder(), state.getCurrentHandPlayerMap(),
					playerRevealOrder);
	...
	public void visit(DealPocketCardsRound round) {
		throw new UnsupportedOperationException(round.getClass()
				.getSimpleName() + " round not allowed in Texas Holdem");
	};
