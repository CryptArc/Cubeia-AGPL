...
import com.cubeia.poker.hand.calculator.TelesinaHandStrengthEvaluator;

public class TelesinaHandComparator implements Comparator<Hand> {

	private TelesinaHandStrengthEvaluator evaluator = new TelesinaHandStrengthEvaluator();
	...
	/**
	 * Set the rank of the lowest card not stripped from the deck
	 * 
	 * @param deckLowestRank
	 */
	public void setDeckLowestRank(Rank deckLowestRank) {
		this.deckLowestRank = deckLowestRank;
	}
	...
	private int compare0(List<Card> c1, List<Card> c2) {
		TelesinaHandStrength c1Strength = evaluator.getStrength(c1, deckLowestRank);
		TelesinaHandStrength c2Strength = evaluator.getStrength(c2, deckLowestRank);
