...
public abstract class GuiceTest extends TestCase {
	...
	protected void setupDefaultGame() {
		...
		PokerSettings settings = new PokerSettings(100, 1000, 10000, TimingFactory.getRegistry().getTimingProfile(MINIMUM_DELAY), TEXAS_HOLDEM, 6, BetStrategyName.NO_LIMIT);
		game = injector.getInstance(PokerState.class);
		game.setServerAdapter(mockServerAdapter);
		game.init(new DummyRNGProvider(), settings);
	}
}
