...
public class BackendCallHandler {
    ...
    public void handleReserveSuccessfulResponse(int playerId, ReserveResponse reserveResponse) {
        ...
        if (state.isPlayerInHand(playerId)) {
            log.debug("player is in hand, adding reserved amount {} as pending", reserveResponse.amountReserved);
            pokerPlayer.addPendingAmount(reserveResponse.amountReserved);
        } else {
            log.debug("player is not in hand, adding reserved amount {} to balance", reserveResponse.amountReserved);
