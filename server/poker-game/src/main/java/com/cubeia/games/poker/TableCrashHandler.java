...
public class TableCrashHandler {
    ...
    public void handleCrashOnTable(AbstractGameAction action, Table table, Throwable throwable) {
        ...
        Collection<PokerPlayer> pokerPlayers = removePlayersFromTable(table);
        
        // 5. close player sessions
        closePlayerSessions(table, pokerPlayers);
    ...
    private Collection<PokerPlayer> removePlayersFromTable(Table table) {
        Collection<PokerPlayer> removedPlayers = new ArrayList<PokerPlayer>();
        
        for (GenericPlayer genericPlayer : table.getPlayerSet().getPlayers()) {
