...
/**
 * Utility class for looking up Accounts as used by the Poker system.
 *
...
public class AccountLookup {
	...
	private LoadingCache<AccountQuery, Long> accountIdCache = CacheBuilder.newBuilder()
            ...
            .build(new CacheLoader<AccountQuery, Long>() {
                public Long load(AccountQuery key) {
                    return remoteLlookupUniqueAccountId(key);
                }
            });
     ...
     *
     * TODO: Change to use AccountQuery
     *
    ...
    public long lookupAccountIdForPlayerAndCurrency(long playerId, String currency) {
        ...
        AccountQueryResult accounts = walletClient.listAccounts(request);
    ...
    /**
     * Look up static player account for given role.
     * 
     * @param walletClient
     * @param operatorId
     * @param currencyCode
     ...
     */
    public long lookupPlayerAccount(long playerId, String currencyCode, AccountRole role) {
    	log.debug("Lookup Player Account. Player["+playerId+"] currency["+currencyCode+"] Role["+role+"]");
    	Map<String, String> attributes = new HashMap<String, String>();
    	attributes.put(AccountAttributes.ROLE.name(), role.name());
    	long accountId = lookupUniqueAccountId(null, playerId, currencyCode, AccountType.STATIC_ACCOUNT, attributes);
    	log.debug("Lookup Player Account. Player["+playerId+"] currency["+currencyCode+"] Role["+role+"], Result: "+accountId);
		return accountId;
    }
    ...
    protected Long remoteLlookupUniqueAccountId(AccountQuery key) {
    	log.debug("Lookup remote account id from key: "+key);
		Account account = walletClient.findUniqueAccount(key);
		if (account == null) {
			throw new NoSuchAccountException("No account matches the query: "+key);
		}
		return account.getId();
	}
